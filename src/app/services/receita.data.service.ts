...
export class ReceitaDataService {
  ...
  listarReceitas(): Observable<Receita[]> {
    return this.http.get<Receita[]>(this.apiUrl).pipe(
      catchError(this.handleError<Receita[]>('listarReceitas', []))
    );
  }

  adicionarReceita(receita: Receita): Observable<Receita> {
    return this.http.post<Receita>(this.apiUrl, receita, this.httpOptions).pipe(
      catchError(this.handleError<Receita>('adicionarReceita'))
    );
  }

  atualizarReceita(id: string,receita: Receita): Observable<any> {
    const url = `${this.apiUrl}/${id}`; 
    return this.http.put(url, receita, this.httpOptions).pipe(
      catchError(this.handleError<any>('atualizarReceita'))
    );
  }
  ...
  excluirReceita(id: string): Observable<Receita> {
    ...
    return this.http.delete<Receita>(url, this.httpOptions).pipe(
      catchError(this.handleError<Receita>('excluirReceita'))
    );
  }
